...
public class World {
    ...
    public void execute(Agent agent) throws InterruptedException {
        ...
        for (Player player : run()) {
            ...
            agent.beforeAction(player);					// 격자에 Player의 rendering 하고 게임정보를 표시한다 
            Action actions = agent.getAction(player); // Agent로 부터 action를 얻는다
            player.setAction(actions);					    // Player는 action를 수행한다
            agent.afterAction(player);						// 화면에 Player rendering하고  
        }
    }
...
//    	setGold(3,2);
    ...
    public void reset() throws InterruptedException {
         ...
         * [과제의 선행 조건]
         * 
         * 출발지 Tile를 제외한 15개의 Tile에 10% 존재한다는 것은 괴물이 1.5개, 웅덩이가 1.5개 이므로 괴물+웅덩이는 3개의 Tile에 존재할 수 있다.
         * 15개의 격자에 괴물2 + 웅덩이1 또는 괴물1 + 웅덩이2 개 존재한다.
         * 
         * 따라서 괴물을 1 ~ 2까지의 범위에서 난수를 선 발생해서 1이면 웅덩이를 2개 만들고, 2이면 웅덩이를 1개만 만들어 15개의 Tile에 난수로 위치시킨다.
         * 선 발생된 Tile위치에 배치한 괴물(또는 웅덩이)와 이 후 위치시키는 좌표가 동일한지 채크해서 동일하면 다른 위치 난수가 발생될 때 까지 반복한다.
         *  
         * --> 아래 setRandom 메쏘드에서 isEmpty로 check하는 과정에서 확인할 수 있다.
         * 
         * 
         */
        int min = 1;
