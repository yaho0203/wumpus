...
public class HeuristicAgent implements Agent {
    ...
    public void afterAction(Player player) {
        if (debug) {
            // Players Last action
            System.out.println(  player.getLastAction() + " : with " + player.getArrows() + " arrows" );
    ...
    public Action getAction(Player player) {
        ...
        if (debug) {
            System.out.format("commanded go to (%d,%d)%n", next[0], next[1]);
        }
    ...
    private int getCost(Player player, int[] to) {
        ...
        if (visited[to[0]][to[1]]) {
            if (player.hasGold() || player.hasArrows() == false) sum -= 5; // 화살이 없는 경우 처음 위치로 원복  
            else sum += 5;
        } else {
